...
def view_map(m, title, min=None, max=None, cmap='YlGnBu_r'):
     ...
     plt.savefig(f'/sharefs/alicpt/users/chenwz/reconstruction_multi_process2/process_added/check_rec_map/{title}.png', dpi=300)
...
def check_and_plot(nside, idx):
    ...
    #pmap_rec = hp.alm2map(hp.read_alm(f'/sharefs/alicpt/users/chenwz/reconstruction_multi_process2/process_added/rec_qlm/rec_alm_{idx}.fits'),nside=2048)
    #pmap_rec = hp.alm2map(hp.read_alm(f'/sharefs/alicpt/users/chenwz/reconstruction_multi_process2/process_added/com_rec/rec_qlm_{idx}.fits'),nside=2048)
    comb_alm = np.load(f"/sharefs/alicpt/users/chenwz/reconstruction_multi_process2/process_added/LSS_tracer/COMB_KIL/comb_KIL_{idx}.npy")
    pmap_rec = hp.alm2map(hp.almxfl(comb_alm[0], cli(q2k(np.arange(3071 + 1)))),nside)
    ...
    view_map((dmap_input_cut - dmap_rec_cut) * mask, title=f' Difference lensing map {idx:04d}',  cmap='YlGnBu_r', min=-0.0024, max=0.0024)

seeds =  [_ for _ in range(0,501)]

#check_and_plot(nside,0)
