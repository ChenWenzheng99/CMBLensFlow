...
def making_maps_new(nside, nrms_f=None,fwhm_f=None,phi_map=None,pixwin=False,lens=True):
    ...
    #unlensed CMB 球谐系数
    if nside != 1024:
        lmax_unl, mmax_unl = 3*2048-1,3*2048-1
    tlm_unl = synalm(cl['tt'][:lmax_unl+1], lmax=lmax_unl, mmax=mmax_unl)
    elm_unl = synalm(cl['ee'][:lmax_unl+1], lmax=lmax_unl, mmax=mmax_unl)
    blm_unl = synalm(cl['bb'][:lmax_unl+1], lmax=lmax_unl, mmax=mmax_unl)

    #Generate the primary CMB map if necessary, and then break
    if lens == False:
        if fwhm_f is not None:
            if len(fwhm_f) == 0 or 1:
                fwhm_f = fwhm_f[0] if fwhm_f else 0.
                transf = bl(fwhm_f, nside=nside, lmax=lmax_unl, pixwin=pixwin)
            else:
                assert nrms_f
                transf = bl_eft(nrms_f, fwhm_f, nside=nside, lmax=lmax_unl, pixwin=pixwin)  #Effective beam function
            tlm_unl = hp.almxfl(tlm_unl, transf, inplace=True)
            elm_unl = hp.almxfl(elm_unl, transf, inplace=True)
            blm_unl = hp.almxfl(blm_unl, transf, inplace=True)
        Tunl = hp.alm2map(tlm_unl, nside, )
        Qunl, Uunl = hp.alm2map_spin([elm_unl, blm_unl], nside, 2, lmax_unl)

        return Tunl, Qunl, Uunl
    ...
    if fwhm_f is not None:
        '''
        Tlen = hp.smoothing(Tlen, fwhm=fwhm)   #用这种方法也可以,为照顾lensing reconstruction而改用下面的方法,注意:这里的smoothing未考虑pixel window function。
...
def making_maps(nside, fwhm=None,phi_map=None):
    ...
    if phi_map is not None:
        pmap = phi_map  
        plm = hp.map2alm(pmap, lmax=dlmax, verbose=False)
    else:
        plm = hp.synalm(cl['pp'],lmax=dlmax,new=True,)
        pmap = hp.alm2map(plm, nside, lmax=dlmax, )

    #unlensed CMB 球谐系数
    tlm_unl = synalm(cl['tt'], lmax=lmax_unl, mmax=mmax_unl)
    elm_unl = synalm(cl['ee'], lmax=lmax_unl, mmax=mmax_unl)
    blm_unl = synalm(cl['bb'], lmax=lmax_unl, mmax=mmax_unl)
