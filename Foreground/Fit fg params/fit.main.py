...
class fg_model:
    ...
    def fg_spectra(self, nu, beta_s, beta_d, Td = 19.6, nu0_s=23*1e9, nu0_d=353*1e9):  #NOTICE :Since our fg map are in KCMB unit, this fg_spectra should also be converted to KCMB unit.
        ...
        Tcmb = 2.7255
        Td = 19.6   #in K
        f_s = (nu/nu0_s) ** beta_s     / (self.unit_convert(nu)/self.unit_convert(nu0_s))                              #If you use uK_CMB unit, you should add this unit conversion.
    ...
    def Dl_fg_auto_nu(self, nu1, nu2, lmax, bin, A_d, A_s, alpha_s, alpha_d, beta_s, beta_d, l0 = 80, if_bin = True):  #Retuen Dl_dd, Dl_ss for single component
        l = np.arange(10000)
    ...
    def Dl_fg_cross_nu(self, nu1, nu2, lmax, bin, A_d, A_s, alpha_s, alpha_d, beta_s1, beta_s2, beta_d1, beta_d2, eps_ds, l0 = 80, if_bin = True):  #Return Dl_ds for two components
        l = np.arange(10000)
        Dl_d = A_d * (l/l0) ** alpha_d
        Dl_s = A_s * (l/l0) ** alpha_s
...
data_vec = np.loadtxt('./data_vec_d9s4_ali_GE.txt')
COV_th = np.load('./COV_matrix_d9s4_ali_GE.npy')           
...
def get_th_vec(nus, bin, A_d, A_s, alpha_s, alpha_d, beta_s, beta_d, eps_ds, dl_bias=None, lmin=0, lmax=200):



    dl_FG_th = get_FG(nus, lmax, bin, A_d, A_s, alpha_s, alpha_d, beta_s, beta_d, eps_ds, if_bin = True)   #更换MODEL，检查
    #dl_cross_de_bias = get_ds_bias(nus, lmax, bin, A_d, A_s, alpha_s, alpha_d, beta_s, beta_d, if_bin = True)
    ...
    for i,_ in nus.items():
        for j,_ in nus.items():
            ...
            key = (i,j)
            if key[0] != key[1]:
                dl_obs[key] = dl_FG_th[key] #+ dl_cross_de_bias[key]     #################注意：这次fg模拟发现没有cross bias项，待检查
            else :
                dl_obs[key] = dl_FG_th[key] #+ dl_cross_de_bias[key]
    
    th_vec = np.array([dl_obs[(27,27)],dl_obs[(27,39)],dl_obs[(27,93)],dl_obs[(27,145)],dl_obs[(27,225)],dl_obs[(27,280)],
            dl_obs[(39,39)],dl_obs[(39,93)],dl_obs[(39,145)],dl_obs[(39,225)],dl_obs[(39,280)],
            dl_obs[(93,93)],dl_obs[(93,145)],dl_obs[(93,225)],dl_obs[(93,280)],
            dl_obs[(145,145)],dl_obs[(145,225)],dl_obs[(145,280)],
            dl_obs[(225,225)],dl_obs[(225,280)],
            dl_obs[(280,280)],])
...
def gauss_cl_logp_th_cov( A_d, A_s, alpha_d,  alpha_s,  beta_s, beta_d, eps_ds, nus=nus,  bin=20, dl_bias=None, lmin=0, lbin_max = 20):
    """
    Defines a gaussian likelihood in log.
    """
    likelihood = 0
    X_theory = get_th_vec(nus, bin, A_d, A_s, alpha_s, alpha_d, beta_s, beta_d, eps_ds, dl_bias=None, lmin=0, lmax=((lbin_max+1)*bin+1))
    for i in np.arange(2,lbin_max):
...
info = {"likelihood": {"r&AL": gauss_cl_logp_th_cov}}

info["params"] = {
    "A_d": {"prior": {"min": 0, "max": 100}, "ref": 6, "proposal": 0.05, "latex": r"A_d"},
    "alpha_d": {"prior": {"min": -1, "max": 0}, "ref": -0.17, "proposal": 0.01, "latex": r"\alpha_d"},
    "beta_d": {"prior": {"min": 0, "max": 3}, "ref": 1.48, "proposal": 0.01, "latex": r"\beta_d"},
    "A_s": {"prior": {"min": 0, "max": 10}, "ref": 1.4, "proposal": 0.05, "latex": r"A_s"},
    "alpha_s": {"prior": {"min": -4, "max": 0}, "ref": -0.95, "proposal": 0.01, "latex": r"\alpha_s"},
    "beta_s": {"prior": {"min": -4, "max": -2}, "ref": -3.1, "proposal": 0.01, "latex": r"\beta_s"},
    "eps_ds": {"prior": {"min": -1, "max": 1}, "ref": 0, "proposal": 0.001, "latex": r"\epsilon_{ds}"},
}
...
# Analyze and plot
mean = full_chain.getMeans()[:7]
covmat = full_chain.getCovMat().matrix[:7, :7]
print("Mean:")
print(mean)
...
gdplot.triangle_plot(full_chain, ["A_d", "alpha_d", "beta_d", "A_s", "alpha_s", "beta_s", "eps_ds"], filled=True, contour_colors=['red'])
...
gdplot.add_text("fg_d9s4_ali_GE, cov_th", x=0, y=3)
gdplot.export('gaussian1.png', dpi=300)
