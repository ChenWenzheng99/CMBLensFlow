...
def gaussian_phi_n0(lmin_cut,lmax_cut,rlmin_cut=[0],rlmax_cut=[0],nside=2048,sigmas=[0],thetas=[0],flag=5,seed=1234):
   ...
   #ucl_clps = cmb.read_camb_cls('/sharefs/alicpt/users/chenwz/download/cmblensplus2/example/data/unlensedcls.dat',ftype='scal',output='array')[:,:rlmax+1]
   # lcl is an array of shape [0:4,0:rlmax+1] and lcl[0,:] = TT, lcl[1,:] = EE, lcl[2,:] = BB, and lcl[3,:] = TE
   #lcl_clps = cmb.read_camb_cls('/sharefs/alicpt/users/chenwz/download/cmblensplus2/example/data/lensedcls.dat',ftype='lens',output='array')[:,:rlmax+1]

   #from Planck FFP10
   cl1 = camb_clfile('/sharefs/alicpt/users/chenwz/download/plancklens/plancklens/data/cls/FFP10_wdipole_lenspotentialCls.dat',rlmax)   #TT,EE,BB,TE,PP,TP,EP
   cl2 = camb_clfile('/sharefs/alicpt/users/chenwz/download/plancklens/plancklens/data/cls/FFP10_wdipole_lensedCls.dat',rlmax)            #TT,EE,BB,TE
   ...
   # 对每个频道进行线性组合
   result_nl = np.sum(weights**2 * nl_list, axis=0) + np.sum(anti_weights * nl_list, axis=0)
