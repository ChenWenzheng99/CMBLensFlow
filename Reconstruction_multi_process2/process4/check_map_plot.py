...
def check_and_plot(nside, idx):
    ...
    pmap_input = hp.read_map(f'/sharefs/alicpt/users/chenwz/reconstruction_2048_simons/test/ALILENS/sims/cmbs/map_P_2048_{idx+300:04d}.fits')
    ...
    #通过截取l范围可知，重建得到的phi在l=8到l=300左右的范围内是相对准确的，这与SNR或者功率谱的计算结果是一致的
    ...
    view_map((dmap_input_cut - dmap_rec_cut) * mask, title=f' Difference lensing map {idx+300:04d}',  cmap='YlGnBu_r', min=-0.0024, max=0.0024)
