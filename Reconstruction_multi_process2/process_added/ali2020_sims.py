...
class simsLensing:    #用于读图，sims分成CMB,NOISE等部分，可以自己添加：residual, foreground, point source等，得到最终模拟的图，对应实际观测图。
    def __init__(self):
        ...
        #self.cmbs = os.path.join( ALILENS_cmb, f'map_TQU_2048_{idx+400:04d}.fits')  #cmbs: path of simulated lensed CMB map  #cmbs: path of simulated lensed CMB map
    ...
    def get_sim_tmap(self, idx):
        T = hp.read_map(self.cmbs % idx, field=0)
        #ac2rad = np.pi/10800.
        T = self.replace_fwhm_map(nside, lmax, T, 1.4, 2.2, True, True)
        noise = hp.read_map( os.path.join(ALILENS_noise, 'map_noise_nside2048_%04d.fits') % idx, field=0) #simulated noise Tmap
    ...
    def get_sim_pmap(self, idx):
        ...
        noise_Q, noise_U = hp.read_map( os.path.join(ALILENS_noise, 'map_noise_nside2048_%04d.fits') % idx, field=(1,2)) #simulated noise Qmap and Umap
