...
# 设置源文件夹和目标文件夹路径
source_folder="/sharefs/alicpt/users/chenwz/reconstruction_multi_process2/process5/ALILENS/temp/qlms_dd"
target_folder="/sharefs/alicpt/users/chenwz/reconstruction_multi_process2/process_added/ALILENS/temp/qlms_dd"

# 确保目标文件夹存在
mkdir -p "$target_folder"

# 遍历文件并复制和重命名
for i in $(seq -f "%04g" 20 119); do
    old_name="${source_folder}/sim_p_${i}.fits"
    new_index=$(printf "%04d" $((10#$i + 380)))
