...
def delensing_ones(nside,nlev,theta_ac,):
    ...
    comb_alm = np.load(f"{comb_alm_filename}")
    ...
    ##############      NOTICE:THESE ARE CONVERGENCE MAPS, YOU MUST CONVERT THEM TO POTENTIAL MAPS FIRST      #############
    COMB_NOISY = hp.alm2map(hp.almxfl(comb_alm[0], cut*cli(q2k(np.arange(dlmax + 1)))),nside)
    COMB_SIGNAL = hp.alm2map(hp.almxfl(comb_alm[1], cut*cli(q2k(np.arange(dlmax + 1)))),nside)  #代替原先filtered signal
    COMB_NOISE = hp.alm2map(hp.almxfl(comb_alm[2], cut*cli(q2k(np.arange(dlmax + 1)))),nside)   #代替原先filtered noise
    ...
    phi_noise = COMB_NOISE

    phi_noisy_part = COMB_NOISY*mask2                                       #noisy phi

    Pmap_part = COMB_SIGNAL*mask2                                             #signal phi

    phi_noise_part = COMB_NOISE*mask2                                       #noise phi
    ...
    #pure signal phi alm
    plm_pure = hp.map2alm(COMB_SIGNAL*mask2,dlmax)
    ...
    Bmap_noisy_origin = E2Bcorrection_new(nside,50,[Qlen_noisy_part,Qlen_noisy_part,Ulen_noisy_part],mask_sm,mask_sm,lmax=4000,flag='clean',cal_cl=False,n_iter=4, is_Dell=True)

    Bmap_noise_origin = E2Bcorrection_new(nside,50,[Qlen_noise_no_fg_part,Qlen_noise_no_fg_part,Ulen_noise_no_fg_part],mask_sm,mask_sm,lmax=4000,flag='clean',cal_cl=False,n_iter=4, is_Dell=True)

    Bmap_fg_origin = E2Bcorrection_new(nside,50,[Qlen_fg_part,Qlen_fg_part,Ulen_fg_part],mask_sm,mask_sm,lmax=4000,flag='clean',cal_cl=False,n_iter=4, is_Dell=True)
...
if __name__ == "__main__":
    ...
    savePath = f'/sharefs/alicpt/users/chenwz/Testarea/FORECAST/FOREGROUND_FORECAST/self_check_temp_comb/auto'
    savePath2 = f'/sharefs/alicpt/users/chenwz/Testarea/FORECAST/FOREGROUND_FORECAST/self_check_temp_comb/cross'
    savePath3 = f'/sharefs/alicpt/users/chenwz/Testarea/FORECAST/FOREGROUND_FORECAST/self_check_temp_comb/de'
