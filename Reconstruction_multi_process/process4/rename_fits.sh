...
# 设置文件夹路径
folder_path="/sharefs/alicpt/users/chenwz/reconstruction_multi_process/process3/ALILENS/temp/qlms_dd"

# 遍历文件夹中的文件并重命名

for i in $(seq -w 200 299); do
    old_name="${folder_path}/sim_p_$(printf "%04d" $i).fits"
    new_index=$(printf "%04d" $((i - 200)))
...
for i in $(seq -w 200 299); do
    old_name="${folder_path}/sim_x_$(printf "%04d" $i).fits"
    new_index=$(printf "%04d" $((i - 200)))
    new_name="${folder_path}/sim_x_${new_index}.fits"
