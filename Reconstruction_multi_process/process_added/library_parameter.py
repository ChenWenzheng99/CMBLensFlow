...
ALILENS = "/sharefs/alicpt/users/chenwz/reconstruction_multi_process/process_added/ALILENS" #the library directory of your dataset and the final output
...
ALILENS_noise = "/sharefs/alicpt/users/chenwz/reconstruction_2048_simons2/test/ALILENS/sims/noise" #the library directory of your dataset and the final output

ALILENS_fg = "/sharefs/alicpt/users/chenwz/Testarea/FORECAST/ALL_MAP_2048/FG/93"

nside = 2048
lmax = 3*nside-1
dlmax = 4096
seeds = [_ for _ in range(500)]  #0到200共201个realization (即simulation)
...
"/sharefs/alicpt/users/chenwz/reconstruction_2048_simons2/I_noise_Simons_2048_inhomo_93.fits"
        ]
fwhm_f = [#19.
         #11.
        #3.
...
bias=500 # how many sets of CMB maps used to calculate mean field  #必须是偶数，因为要计算两个平场，44 sets 平分。且平分之后最好也是偶数，否则可能引入bias.
var=0 # how many sets of CMB maps used to calculate covariance matrix
nset=10 # the number of group  
nwidth=50 # the width of each group (how many sets in each group)  #注：每组内进行shuffle, 即让 j=i+1,且cyclically，见params.py 的 ss_dict
