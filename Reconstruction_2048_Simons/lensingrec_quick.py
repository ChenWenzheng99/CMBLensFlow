...
for qe_key in ['p', ]:
    for idx in range(0, len(seeds)):
        ...
        print('Finish calculating qlm %d' % idx)
"""
"""
...
for qe_key in ['p', ]:
    ...
    ll = np.arange(lmax + 1)
    np.savetxt(f"/sharefs/alicpt/users/chenwz/reconstruction_2048_simons/wiener.dat", [ll, nhl, clpp[:lmax + 1] + nhl])
...
for qe_key in ['p', ]:
    ...
    for idx in range(0, len(seeds)):
        ...
        hp.write_alm(f"/sharefs/alicpt/users/chenwz/reconstruction_2048_simons/rec_qlm/rec_alm_{idx}.fits", qlm_rec, overwrite=True)
