...
def lensed_cmbs(cls, nside, savePath, fwhm_f=[], nrms_f=None, lmax=4096, dlmax=1024, facres=-1, seed=0):
    """ Lensed CMB TQU maps random realization.

         * cls      : dict which contains input power spectra
         * nside    : nside
    ...
    def simulate_and_save_maps()
        ...
        if len(fwhm_f) == 0 or len(fwhm_f) == 1:
    ...
    if not ismap_exist:
        ...
        return #结束函数
    # 如果 pmap_exist 中存在元素大于1，则继续执行；否则结束函数
    elif np.any(pmap_exist > 1) or np.any(tmap_exist > 1000) or np.any(qmap_exist > 100) or np.any(umap_exist > 100) or not ismap_exist:
        simulate_and_save_maps()
    else:
