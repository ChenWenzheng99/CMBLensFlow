...
def get_th_vec(r, AL, noise, nus, bin, A_d, A_s, alpha_s, alpha_d, beta_s, beta_d, eps_ds, dl_bias=None, lmin=0, lmax=100):
    ...
    for i,_ in nus.items():
        for j,_ in nus.items():
            ...
            key = (i,j)
            if key[0] != key[1]:
                dl_obs[key] = dl_tens_r + AL * dl_lens + noise[key] + dl_FG_th[key] #+ dl_cross_de_bias[key]     #该fg map似乎不需要这项，待检查
            else :
...
##############################################  Cut Cov matrix ######################################################
data_cut = [6,12,17,21,24,26,27]
for i in data_cut:
    COV_sim[:,i,:] = 0
    COV_sim[:,:,i] = 0

# 初始化一个列表来保存处理后的矩阵
COV_sim_reduced_list = []

for i in range(COV_sim.shape[0]):
    # 查找全为零的行和列的索引
    rows_to_keep = ~np.all(COV_sim[i] == 0, axis=1)
    cols_to_keep = ~np.all(COV_sim[i] == 0, axis=0)
    
    # 删除全为零的行和列
    reduced_matrix = COV_sim[i][rows_to_keep][:, cols_to_keep]
    
    # 将处理后的矩阵添加到列表中
    COV_sim_reduced_list.append(reduced_matrix)

# 将列表转换为三维数组
# 由于不同 l 的协方差矩阵大小不同，无法直接转换为普通的 numpy 数组，因此保留为列表格式
COV_sim_reduced = COV_sim_reduced_list

##############################################  Define likelihood ###################################################
COV_INV = np.linalg.inv(COV_sim_reduced)
COV_DET = np.log(np.linalg.det(COV_sim_reduced))
...
def gauss_cl_logp_sim_cov(r,AL,  A_d, A_s, alpha_d, alpha_s,  beta_s, beta_d, dl_noise=dl_noise, nus=nus, eps_ds=0, bin=20, dl_bias=None, lmin=0, lbin_max = 10):
    ...
    for i in np.arange(1,lbin_max):
        likelihood_l = np.dot(np.dot((data_vec_cut[:,i] - X_theory[:,i]).T, COV_INV[i,:,:] ), (data_vec_cut[:,i] - X_theory[:,i])) + COV_DET[i]
        #likelihood_l = np.dot(np.dot((X_data[:,i] - X_theory[:,i]).T, cli(cal_cov_3(X_data,l[i],fsky=0.8,)[:,:,i])), (X_data[:,i] - X_theory[:,i])) 
        likelihood += -0.5*likelihood_l
    return likelihood
...
info["params"] = {
    ...
    "alpha_s": {"prior": {"min": -1.4, "max": -0.1}, "ref": -0.80, "proposal": 0.003, "latex": r"\alpha_s"},
    "beta_s": {"prior": {"min": -3.3, "max": -2.9}, "ref": -3.1, "proposal": 0.001, "latex": r"\beta_s"},
}
...
# 保存 full_chain
with open('full_chain_obs.pkl', 'wb') as f:
...
gdplot.add_text("obs_sim, cov_sim, ", x=0, y=3)
gdplot.export('gaussian1_cheat_20_200.png', dpi=200)
