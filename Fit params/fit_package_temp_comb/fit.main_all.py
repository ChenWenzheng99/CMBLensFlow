...
class fg_model:
    ...
    def fg_spectra(self, nu, beta_s, beta_d, Td = 19.6, nu0_s=23*1e9, nu0_d=353*1e9):  #NOTICE :Since our fg map are in KCMB unit, this fg_spectra should also be converted to KCMB unit.
        ...
        Tcmb = 2.7255
        Td = 19.6   #in K
        f_s = (nu/nu0_s) ** beta_s      / self.unit_convert(nu)                             #If you use uKCMB unit, you should add this unit conversion.
        f_d = (nu/nu0_d) ** (beta_d + 1) * (np.exp(h*nu0_d/k/Td)-1) / (np.exp(h*nu/k/Td)-1)     / self.unit_convert(nu)
        return f_s, f_d
...
data_vec_cut = np.array([data_vec[0],data_vec[1],data_vec[2],data_vec[3],data_vec[4],data_vec[5],
                data_vec[7],data_vec[8],data_vec[9],data_vec[10],data_vec[11],
                data_vec[13],data_vec[14],data_vec[15],data_vec[16],
                data_vec[18],data_vec[19],data_vec[20],
                data_vec[22],data_vec[23],
                data_vec[25]])


dl_lens = np.loadtxt('./LT_LT_mean.txt')[4]
...
def get_th_vec(r, AL, noise, nus, bin, A_d, A_s, alpha_s, alpha_d, beta_s, beta_d, eps_ds, dl_bias=None, lmin=0, lmax=100):
    ...
    for i,_ in nus.items():
        for j,_ in nus.items():
            ...
            key = (i,j)
            if key[0] != key[1]:
                dl_obs[key] = dl_tens_r + AL * dl_lens + noise[key][0] + dl_FG_th[key] #+ dl_cross_de_bias[key]     #该fg map似乎不需要这项，待检查
            else :
                dl_obs[key] = dl_tens_r + AL * dl_lens + noise[key][0] + dl_FG_th[key]
    for i,_ in nus.items():
...
with open('./full_chain_lens.pkl', 'rb') as f:
    full_chain1 = pickle.load(f)

with open('./full_chain_obs.pkl', 'rb') as f:
    full_chain2 = pickle.load(f)   

# 获取第一个 full_chain 的均值和协方差矩阵
mean1 = full_chain1.getMeans()[:8]
covmat1 = full_chain1.getCovMat().matrix[:8, :8]
print("Mean for full_chain1:")
print(mean1)
print("Standard error for full_chain1:")
print(np.sqrt(np.diag(covmat1)))

# 获取第二个 full_chain 的均值和协方差矩阵
mean2 = full_chain2.getMeans()[:8]
covmat2 = full_chain2.getCovMat().matrix[:8, :8]
print("Mean for full_chain2:")
print(mean2)
print("Standard error for full_chain2:")
print(np.sqrt(np.diag(covmat2)))

# %matplotlib inline  # Uncomment if running from a Jupyter notebook

gdplot = gdplt.get_subplot_plotter()
gdplot.triangle_plot([full_chain1, full_chain2], 
                     ["r", "AL", "A_d", "alpha_d", "beta_d", "A_s", "alpha_s", "beta_s"], 
                     filled=False, 
                     contour_colors=['red', 'black'],
                     legend_labels=['After adding LT','Before adding LT', ],
                     legend_kwargs={'fontsize': 20})
"""
for i in range(8):
    # 在对角线上绘制第一个 full_chain 的均值和标准误差
    gdplot.subplots[i, i].axvline(x=mean1[i], color='red', linestyle=':')
    gdplot.subplots[i, i].text(mean1[i], 1, f"Mean1: {mean1[i]:.6f}", fontsize=6, color='red', ha='left', va='top')
    gdplot.subplots[i, i].text(mean1[i], 0.9, f"Sigma1: {np.sqrt(covmat1[i, i]):.6f}", fontsize=6, color='red', ha='left', va='top')
    
    # 在对角线上绘制第二个 full_chain 的均值和标准误差
    gdplot.subplots[i, i].axvline(x=mean2[i], color='blue', linestyle=':')
    gdplot.subplots[i, i].text(mean2[i], 0.8, f"Mean2: {mean2[i]:.6f}", fontsize=6, color='blue', ha='left', va='top')
    gdplot.subplots[i, i].text(mean2[i], 0.7, f"Sigma2: {np.sqrt(covmat2[i, i]):.6f}", fontsize=6, color='blue', ha='left', va='top')
"""
gdplot.add_text("all_sim_th, cov_sim", x=0, y=3)
