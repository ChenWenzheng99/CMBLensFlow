...
# wiener filter
wiener_dat = np.loadtxt('/sharefs/alicpt/users/chenwz/reconstruction_2048_simons2/wiener.dat')
wiener = (wiener_dat[2] - wiener_dat[1]) * utils.cli(wiener_dat[2])

# input deflection map
qlm_input = hp.map2alm(hp.read_map(os.path.join(ALILENS_cmb, 'map_P_2048_0099.fits')))   #最后一个set作为data
dlm_input = hp.almxfl(qlm_input, cut * q2d(np.arange(lmax + 1)))
dmap_input = hp.alm2map(dlm_input, nside=2048)

# reconstruction map
qlm_recon = hp.read_alm('/sharefs/alicpt/users/chenwz/reconstruction_2048_simons/rec_qlm/rec_alm_99.fits')
dlm_recon = hp.almxfl(qlm_recon, cut * q2d(np.arange(lmax + 1) * wiener))
dmap_recon = hp.alm2map(dlm_recon, nside=2048)
"""
# reconstruction mean-field map
qlm_mf = par.qlms_dd.get_sim_qlm_mf('p', par.mc_sims_mf_dd)
qresp = par.qresp_dd.get_response('p', 'p')
klm_mf = hp.almxfl(qlm_mf, utils.cli(qresp) * q2k(np.arange(lmax + 1)))
