...
def pysm_generator(nside, comps, freq, unit="uK_CMB", cordinate='G'):
    """
    Generate the foreground maps using PySM3
    ...
    comps : list
        List of components to be included in the map. (e.g. ["d9", "s4", "a1", "f1", "cib1", "tsz1", "ksz1", "d1","s1", "f1", "cib1", "tsz1"])
        Check PySM3 documentation for more details : https://pysm3.readthedocs.io/en/latest/index.html#models
    freq : list
        Frequencies at which the maps are to be generated, should be in GHz

    unit : str
        Unit of the map, should be either "uK_CMB" or "uK_RJ"

    cordinate : str
        Cordinate system of the map, should be either "G"/"GC" (galactic cordinate) or "C"/"GE" (celestial cordinate)

    -------
    Notice:
    ****************** This needs a network connection to download the required fg templates for the first time. (Maybe quite slow) ******************
    """

    sky = pysm3.Sky(nside=nside, preset_strings=comps) 

    map = sky.get_emission(freq * u.GHz)   # This is in uK_RJ, GE cordinate

    if unit == 'uK_CMB':
        map = map.to(u.uK_CMB, equivalencies=u.cmb_equivalencies(freq * u.GHz))

    if cordinate != 'G' :
        map = pysm3.apply_smoothing_and_coord_transform(map, rot=hp.Rotator(coord="G"+cordinate))  # Convert to desired cordinate
...
class fg_model:
    ...
    def Dl_fg_nu(self, lmax, bin, A, alpha, l0 = 80, if_bin = True):  # We only consdier BB power spectrum here.
        ...
        if if_bin == 1:
            _, output = get_binned(2048,bin,Dl[:lmax+1],lmax)
        else:
            output = Dl
        return output
    ...
    def Dl_fg_auto_nu(self, nu1, nu2, lmax, bin, A_d, A_s, alpha_s, alpha_d, beta_s, beta_d, l0 = 80, if_bin = True):  #Retuen Dl_dd, Dl_ss for single frequency
        ...
        if if_bin == 1:
            lbin, output1 = get_binned(2048,bin,Dl_dd[:lmax+1],lmax)
            lbin, output2 = get_binned(2048,bin,Dl_ss[:lmax+1],lmax)
        else:
    ...
    def Dl_fg_cross_nu(self, nu1, nu2, lmax, bin, A_d, A_s, alpha_s, alpha_d, beta_s1, beta_s2, beta_d1, beta_d2, eps_ds, l0 = 80, if_bin = True):  #Return Dl_ds for two (or one) frequency
        ...
        if if_bin == 1:
            lbin, output = get_binned(2048,bin,Dl_cross[:lmax+1],lmax)
        else:
...
def get_fg_cross(nu1, nu2, lmax, bin, A_d, A_s, alpha_s, alpha_d, beta_s1, beta_s2, beta_d1, beta_d2, eps_ds, l0 = 80, if_bin = False):      #Eq(2.9)
    fg = fg_model()
    lbin, Dl_ds = fg.Dl_fg_cross_nu(nu1, nu2, lmax, bin, A_d, A_s, alpha_s, alpha_d, beta_s1, beta_s2, beta_d1, beta_d2, eps_ds, l0 = l0, if_bin = if_bin)
    return lbin, Dl_ds
...
def gaussian_fg(nside, nus, seed, same=False, method=1, A_d_b = 14.30, alpha_d_b = -0.65, A_s_b = 2.40, alpha_s_b = -0.80, beta_d = 1.48, beta_s = -3.10, eps_ds = 0,
    ...
    """
    Generate the Gaussian foreground realizations on the fly.

    So far we do not consider the correlation between dust and synchrotron in the map generation, i.e., eps_ds = 0.
    This is because the correlation only affects a very small part of the total fg power spectrum, and including it will make the map generation much more complicated.

    Parameters
    * nus: All the frequencies you want to use in a single simulation.
    * seed: Random seed for the fg map generation.
    * same: If True, use the same random seed for dust and synchrotron generation, leading to fully correlated dust and synchrotron maps.
    ...
    WARNING: The default fg params. are fitted from PySM3 simulation for ALI-SAT sky. Replace them if you want to simulate other sky fg.
    """
    ...
    #I  # We fit the 7-parameters for Ali sky fg intensity, finding that A_d_t0 = 220 and A_s_t0 = 5.1 with other paramters almost the same as the pol ones.
    A_d_t0 = A_d_t  
    alpha_d_t0 = alpha_d_t
    A_s_t0 = A_s_t  
    alpha_s_t0 = alpha_s_t

    #B modes # For Ali sky, the 7-params for fg pol are: A_d_b0 = 14.3, alpha_d_b0 = -0.65, beta_d0 = 1.48, A_s_b0 = 2.4, alpha_s_b0 = -0.8, beta_s0 = -3.1, eps_ds0 = 0.001(we fixed to 0 in our procedure).
    A_d_b0 = A_d_b
    alpha_d_b0 = alpha_d_b
    A_s_b0 = A_s_b
    alpha_s_b0 = alpha_s_b

    #E modes : The same order of the B-modes, only a little larger.
    A_d_e0 = A_d_e 
    alpha_d_e0 = alpha_d_e
    A_s_e0 = A_s_e
    alpha_s_e0 = alpha_s_e
    ...
    for i,nu in nus.items():
        if method == 1:
            ...
            _,Dl_dust_TT = get_fg_total(nu, nu, lmax, 1, A_d_t0, 0, 0, alpha_d_t0, 0, beta_d0, eps_ds0, l0 = 80, if_bin = False)
            ...
            _,Dl_synch_TT = get_fg_total(nu, nu, lmax, 1, 0, A_s_t0, alpha_s_t0, 0, beta_s0, 0, eps_ds0, l0 = 80, if_bin = False)
            ...
            Dl_synch_EE[0] = 0
            Dl_synch_BB[0] = 0
            Dl_total_TT = Dl_dust_TT + Dl_synch_TT
            ...
            m_sigma_G1 = hp.synfast([
                ...
                zeros[:lmax+1], zeros[:lmax+1], zeros[:lmax+1]], nside, new=True)   
            
            m_sigma_G1_T = hp.synfast(Dl_dust_TT[1:lmax+1] / cl_norm[1:lmax+1], nside, new=True)   #避免l=0的nan
            ...
            if same == True:
                ...
                np.random.seed(seed + np.random.randint(1,10000000))
            m_sigma_G2 = hp.synfast([
                zeros[:lmax+1],
                Dl_synch_EE[:lmax+1]  / cl_norm[:lmax+1], Dl_synch_BB[:lmax+1]  / cl_norm[:lmax+1],
                zeros[:lmax+1], zeros[:lmax+1], zeros[:lmax+1]], nside, new=True) 
            m_sigma_G2_T = hp.synfast(Dl_synch_TT[1:lmax+1]  / cl_norm[1:lmax+1], nside, new=True)   #避免l=0的nan
            ...
            _,Dl_total_TT = get_fg_total(nu, nu, lmax, 1, A_d_t0, A_s_t0, alpha_s_t0, alpha_d_t0, beta_s0, beta_d0, eps_ds0, l0 = 80, if_bin = False)
            ...
            m_sigma_G_T = hp.synfast(Dl_total_TT[1:lmax+1] / cl_norm[1:lmax+1], nside, new=True)
