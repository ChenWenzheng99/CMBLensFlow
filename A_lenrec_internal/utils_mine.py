...
import sys
sys.path.append('/home/chen/software/cmblensplus2')
sys.path.append('/home/chen/software/cmblensplus2/utils')
import numpy as np
#import basic
#import pickle
import curvedsky
...
def coord_trans(mask, coord_in="C", coord_out="G"):
    """
    Transform mask from coord_in to coord_out.
    "C" for equatorial, "G" for Galactic, and "E" for ecliptic.
    Defualt: The Ali-SAT mask is in C coordinates, while the Planck, Ali-LAT and SO-LAT masks are in G coordinates.
    """
    ...
    if coord_in == coord_out:
        ...
        mask_raw = pysm3.apply_smoothing_and_coord_transform(mask, rot=hp.Rotator(coord=(coord_in + coord_out)))
        mask_out = np.where(mask_raw >0.1, 1, 0)
    return mask_out
...
def lm_healpix2healpy(alm2d, lmax=None):
    """ Convert 2D alm array to 1D alm array used in healpy.

        * alm2d : 2D alm array with shape (lmax+1, lmax+1)
        * lmax  : lmax
    """
    # get the lmax
    if lmax is None:
...
def get_pseudo_cl(nside,mask_input,flag,bin,lmax=None,apo=False,f0=None,f2=None,is_Dell=True):
    """
        ...
        nside: HEALPix nside of input maps.
        mask_input: Should be an apodized mask, otherwise set apo=True to do apodization.
        flag: '00' '02' '22', indicating spin-0 x spin-0, spin-0 x spin-2, spin-2 x spin-2 respectively.
        bin: Number of multipoles per bin
        lmax: Maximum multipole for pseudo-Cl calculation, default is -1, which uses 3*nside-1
        apo: Whether the input mask is apodized
        f0: spin-0 field
        f2: spin-2 field
    """
    ...
    if apo == True:
        ...
        mask = nmt.mask_apodization(mask_input,  0.5, apotype="Smooth")           # Apodize the mask with a 0.5 degree smoothing scale
...
def E2Bcorrection_new(nside,bin,map,binary_mask,apo_mask,lmax=None,flag='clean', cal_cl=False, n_iter=3, is_Dell=True, method='cutqufitqu',):
    """
    Correct EB-leakage for partial sky Q/U maps. Input partial sky TQU maps, output TEB maps, with EB-leakage corrected.
    Parameters
        nside : HEALPix nside of input maps.
        lmax: Only used for calculating pseudo-Cl
        bin: Only used for calculating pseudo-Cl
        map: TQU maps with mask on
        binary_mask: 0-1 binary mask, used for EB leakage correction
        apo_mask: apodized mask, used for calculating pseudo-Cl
        n_iter: Number of iterations for EB leakage correction
        method:     'zzr'
    ...
    if lmax is None:
        lmax = 3*nside - 1
    obj = EBLeakageCorrection(map, lmax=lmax, nside=nside, mask=binary_mask, post_mask=binary_mask, method = method, check_res=False, n_iter=n_iter)
...
def map2cl_1024(Q,U,mask_1024,bin,apo=False):
    """
    Convert 1024 Q/U map to pseudo-Cl (EE is cl[0],BB is cl[3]).(for partial sky QU only)
    """
...
def map_degrade(nside_in,nside_out,map_in,lmax_in=None,lmax_out=None):
    """
    Degrade 2048 map to 1024 map。Safe at \ell < 2000, map_in should be TQU.
    2048 map_in -> alm -> 1024 map_out
    """
    if lmax_in is None:
        lmax = 3*nside_in - 1
    else:
...
def Wiener_filter_QU(nside,lmax,noisy_maps,noise,):
    ...
    elm_sig,_ = curvedsky.utils.hp_map2alm_spin(nside, lmax, lmax, 2, noisy_maps[1]-noise[1], noisy_maps[2]-noise[2])
    clee_sig = curvedsky.utils.alm2cl(lmax, elm_sig, alm2=None)

    ratio = (clee_sig) * cli(clee_len_full_noisy)
    ratio[np.where(ratio > 1)] = 1
    return ratio
...
def alm2cl(alm,mask_apo,lmax=None,is_Dell=True):
    ...
    if lmax is None:
        lmax = 3*nside_cut - 1
    _,l1,cl_de = E2Bcorrection_new(1024,bin,[Q,Q,U,],mask_apo,lmax=lmax,flag='clean',n_iter=3, is_Dell=is_Dell)
    return l1,cl_de
