...
def get_blbubc(bin_type):  #用于根据不同的bin_type返回对应的带宽边界和中心
    if bin_type == 'consext8':
        bins_l = np.array([8, 41, 85, 130, 175, 220, 265, 310, 355])                      #######################为啥#########################
        ...
        bins_l = np.array([8, 21, 40, 66, 101, 145, 199, 264, 339, 426, 526, 638, 763, 902, 1549, 2049, 3072, 4001,])
        bins_u = np.array([20, 39, 65, 100, 144, 198, 263, 338, 425, 525, 637, 762, 901, 1548, 2048, 3071, 4000, 5000 ])
    elif '_' in bin_type:  
        edges = np.int_(bin_type.split('_'))  #对于 bin_type 包含下划线 '_' 的情况,它会将 bin_type 拆分成一系列整数
        bins_l = edges[:-1]
        bins_u = edges[1:] - 1
        bins_u[-1] += 1
    else:
        assert 0, bin_type + ' not implemented'
    return bins_l, bins_u, 0.5 * (bins_l + bins_u)   #下边界,上边界,band中心
...
class ffp10_binner:
    """Band-power construction library using the FFP10 fiducial cosmology.
        ...
        CN EDITION:
        类名:ffp10_binner
        描述:这是一个用于使用FFP10标准宇宙学模型的频带构建库。它用于将引力透镜(或各向异性)估计的谱组合,以与Planck 2018引力透镜分析完全相同的方式构建频带(band-powers)。
        功能概述:类中定义了各种方法，如 MCN0、RDN0 等，用于构建带宽的交叉功率谱、计算偏差、获得 MC 和点源修正以及协方差矩阵.
                使用参数文件中定义的各种 QE(二次估计器)和 QE 功率谱库,特别是
                qcls_dd(用于data band-powers,用于构建covariance matrix, Monte-Carlo and point-source correction)
                qcls_ds(用于 RDN0 and point-source correction)
                qcls_ss(用于 MCN0, RDN0, Monte-Carlo and point-source correction)
                qresp_dd(用于估计器的归一化)
                n1_dd(用于 N1 bias subtraction)
                nhl_dd(用于构建semi-analytical covariance matrix)
                ivfs(用于 N1 bias and point-source correction)
        参数说明:
            k1:第一个二次估计器的键(key),具体定义在qest.py模块中。见下面
            k2:第二个二次估计器的键(key),具体定义在qest.py模块中。
            parfile:包含相关估计器库定义的参数文件。即params.py
            btype:波段类型描述符,通常是'consext8'或'arg2',是Planck 2018引力透镜分析的默认选项。
            ksource:各向异性源的标识,默认为'p',表示引力透镜。见下面
        使用说明:这个类可以用于构建两个各向异性估计器的交叉功率谱,计算偏差,获得Monte Carlo和点源校正以及协方差矩阵。
        参数详细说明:文档字符串提供了对每个参数的简要说明,建议参考qest.py模块中的具体定义以获取更多信息。
        注意事项:文档字符串提到了如果相关的估计器、估计器谱等预先计算的数据无法找到,将在需要时进行动态计算。因此,建议在实际配置中提前构建这些数据,
                通过运行一个名为 run_qlms.py 的示例脚本或其他适当的工具来提前计算和存储所需的数据。
        ...
        Anisotropy source keys ('ksource') are a one-letter string including  
            'p' (lensing gradient) 
            'x' (lensing curl) 
            's' (point sources) 
            'f' (modulation field) 
        ...
        Typical keys include then: ( QE anisotropy key, 'k' )
    ...
    def __init__(self, k1, k2, parfile, btype, ksource='p'):
        ...
        lmaxphi = 5000
        cls_path = os.path.join(os.path.dirname(os.path.abspath(plancklens.__file__)), 'data', 'cls') #指向plancklens模块的数据文件夹中的cls子文件夹的路径

        if ksource == 'p':   #kswitch是clpp和clkk转换的系数(l(l+1))^2/2/pi
        ...
        qc_resp = parfile.qresp_dd.get_response(k1, ksource)[:lmaxphi+1] * parfile.qresp_dd.get_response(k2, ksource)[:lmaxphi+1]  #response相当于归一化因子
        bin_lmins, bin_lmaxs, bin_centers = get_blbubc(btype)
        vlpp_inv = qc_resp * (2 * np.arange(lmaxphi + 1) + 1) * (0.5 * parfile.qcls_dd.fsky1234)  #vlpp是binning时的最小方差weight,(V_L)^-1 = (2L+1) * fsky * (R_L)^2 (见Planck2018)
        vlpp_inv *= utils.cli(kswitch) ** 2
        vlpp_den = [np.sum(clkk_fid[slice(lmin, lmax + 1)] ** 2 * vlpp_inv[slice(lmin, lmax + 1)]) for lmin, lmax in zip(bin_lmins, bin_lmaxs)]  #PLANCK2018(11)式分母

        fid_bandpowers = np.ones(len(bin_centers))  # We will renormalize that as soon as l_av is calculated. #得到区间加权平均点lav后用插值估算

        def _get_bil(i, L):  # Bin i window function to be applied to cLpp-like arrays as just described  #见PLANCK2018(11)式
            ret = (fid_bandpowers[i] / vlpp_den[i]) * vlpp_inv[L] * clkk_fid[L] * kswitch[L]
            ret *= (L >= bin_lmins[i]) * (L <= bin_lmaxs[i])  #条件语句,限制L范围:L^b_min <= L <= L^b_max
            return ret
        
        #计算第i区间L值的加权平均值L_b
        lav = np.zeros(len(bin_centers))  
        for i, (lmin, lmax) in enumerate(zip(bin_lmins, bin_lmaxs)):
            w_lav = 1. / np.arange(lmin, lmax + 1) ** 2 / np.arange(lmin + 1, lmax + 2) ** 2    #w_lav为权:1/(l(l+1))^2
            lav[i] = np.sum(np.arange(lmin, lmax + 1) * w_lav * _get_bil(i, np.arange(lmin, lmax + 1))) / np.sum(    #L_b = (\Sigma l*B_l)/(\Sigma B_l),见PLANCK2018(12)式
        ...
        self.fid_bandpowers =  np.interp(lav, np.arange(lmaxphi + 1, dtype=float), clkk_fid) #使用插值技术,根据给定的l和clkk_fid的离散点值,在lav位置(x坐标)估算该点的clkk(y值)
        self.bin_lmins = bin_lmins
        self.bin_lmaxs = bin_lmaxs
        self.bin_lavs = lav    
        self.nbins = len(bin_centers)  #number of bins
    ...
    def _get_bil(self, i, L):   #binning function,即B_b^L，见PLANCK 2018 (11)式
    ...
    def _get_binnedcl(self, cl):
        ...
        for i, (lmin, lmax) in enumerate(zip(self.bin_lmins, self.bin_lmaxs)):
            ret[i] = np.sum(self._get_bil(i, np.arange(lmin, lmax + 1)) * cl[lmin:lmax + 1])  #band-power estimate :\hat C_Lb^{kk} = \Sigma{L} B_b^L * \hat C_L^{kk}
        return ret                                                                              #见PLANCK 2018 (10)式
    ...
    def get_mcn0(self):      #MCN0和RDN0只是算法的区别,是(解析的)N0算法上的近似实现,见PLANCK2018(A.1) 
        """Returns Monte-Carlo N0 lensing bias.

        """
        ss = self.parfile.qcls_ss.get_sim_stats_qcl(self.k1, self.parfile.mc_sims_var, k2=self.k2).mean() #get_sim_stats_qcl: Returns the average of QE power spectra(看其代码好像只是各个cl向量对应元素求和了)
        qc_resp = self.parfile.qresp_dd.get_response(self.k1, self.ksource) * self.parfile.qresp_dd.get_response(self.k2, self.ksource)
        return self._get_binnedcl(utils.cli(qc_resp) * (2. * ss))    #为什么有response?因为每一步的ss、ds都没有考虑我们之前的mask、filter等因素的影响，只有最后一步才进行归一化。也可见get_dat_bandpowers().
    ...
    def get_dat_nhl(self):
        ...
        """
        qc_resp = self.parfile.qresp_dd.get_response(self.k1, self.ksource) * self.parfile.qresp_dd.get_response(self.k2, self.ksource)
        return self._get_binnedcl(utils.cli(qc_resp) * self.parfile.nhl_dd.get_sim_nhl(-1, self.k1, self.k2))    #get_sim_nhl定义见plancklens的nhl.py文件
    ...
    def get_n1(self, k1=None, k2=None, unnormed=False):
        ...
        assert np.all(self.parfile.qcls_dd.qeA.f2map1.ivfs.get_ftl() == self.parfile.qcls_dd.qeA.f2map2.ivfs.get_ftl())  #inverse-variance filtered CMB spectra
        ...
        ftlA = ivfsA.get_ftl()  #见/home/rabbit/settings/plancklens/plancklens/filt/filt_util.py  line 151
        felA = ivfsA.get_fel()
        fblA = ivfsA.get_fbl()
        ftlB = ivfsB.get_ftl()
        felB = ivfsB.get_fel()
        ...
        n1pp = self.parfile.n1_dd.get_n1(k1, self.ksource, clpp_fid, ftlA, felA, fblA, len(qc_resp) - 1,   #get_n1定义见plancklens的n1.py文件
                                         kB=k2, ftlB=ftlB, felB=felB, fblB=fblB)
        return self._get_binnedcl(utils.cli(qc_resp) * n1pp) if not unnormed else n1pp
        #HOW TO FIND  defination of get_ftl():
        #parfile -> params.py
        #qcls_** = qecl.library(libdir_qcls_**, qlms_**, qlms_**, mc_sims_mf_**) ,第二和第三参数为qeA和qeB,见qecl.py class library
        #qlms_** = qest.library_sepTP(libdir_qlms_**, ivfs, ivfs,   cl_len['te'], nside, lmax_qlm=lmax_qlm),第二和第三参数为ivfs,见qest.py class library
        #ivfs定义在params.py, ivfs = filt_util.library_ftl(ivfs_raw, lmax_ivf, ftl_rs, fel_rs, fbl_rs),故见filt_util.py class library_flt
    ...
    def get_bamc(self):
        ...
        for i, idx in utils.enumerate_progress(self.parfile.mc_sims_var, label='collecting BP stats'):
            ...
            bp_stats.add(self._get_binnedcl(qc_norm *(dd - ss2) - cl_pred) - bp_n1) #见PLANCK 2018 (A.2)
        NMF = len(self.parfile.qcls_dd.mc_sims_mf)   # the number of simulations entering the mean-field subtraction,见PLANCK 2018 (13)式
        NB = len(self.parfile.mc_sims_var)           # the number of simulations used for the noise biases and MC correction.
        return bp_stats.mean(), bp_stats.sigmas_on_mean() * np.sqrt((1. + 1. + 2. / NMF + 2 * NB / (float(NMF * NMF))))   #没找到公式
    ...
    def get_bmmc(self, mc_sims_dd=None, mc_sims_ss=None):
        ...
        # NOTE N1 calculation can not be parallel so for speed we can have @Jinyi
        bps = self._get_binnedcl(utils.cli(qc_resp) * (dd - 2 * ss) - cl_pred[:len(dd)])  #没找到公式
        return 1. / (1 + bps / self.fid_bandpowers)

#下面这些是计算covariance不同的来源的贡献（之前Cl要-N0-N1-MC-PS等，这些都会贡献），最后把他们线性求和得到covariance matrix,文章一般不写每一部分公式，只写最终的。(见PLANCK 2018 附录correction部分)
    ...
    def get_nhl_cov(self, mc_sims_dd=None):
        ...
        for i, idx in utils.enumerate_progress(mc_sims_dd):
            dd = self.parfile.qcls_dd.get_sim_qcl(self.k1, idx, k2=self.k2)
            nhl_cov.add(self._get_binnedcl(qc_norm * (dd- self.parfile.nhl_dd.get_sim_nhl(int(idx), self.k1, self.k2))))  #没找到公式
        return nhl_cov.cov()
    ...
    def get_mcn0_cov(self, mc_sims_dd=None):
        ...
        for i, idx in utils.enumerate_progress(mc_sims_dd):
            dd = self.parfile.qcls_dd.get_sim_qcl(self.k1, idx, k2=self.k2)   
            mcn0_cov.add(self._get_binnedcl(qc_norm * dd))    #没找到公式
        return mcn0_cov.cov()
    ...
    def get_bmmc_cov(self, mc_sims_dd=None):
        ...
        """
        return np.outer(np.sqrt(np.absolute(self.get_bmmc())), np.sqrt(np.absolute(self.get_bmmc())))    #没找到公式

    def get_cov(self):
        """Covariance matrix obtained from MCN0 corrected by mMC.

        """
        return self.get_mcn0_cov() * self.get_bmmc_cov()    #没找到公式
