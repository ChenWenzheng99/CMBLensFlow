...
def lensingb_mine(lmax, elmin, elmax, plmin, plmax, wElm, wplm, nside_t=0, gtype='p',sm_mask=None,bi_mask=None):
    """
    ...
    if bi_mask is None:
        bi_mask = np.ones(npix, dtype=float)
    ...
    map_b = E2Bcorrection_new(nside,50,[map[:,0]*bi_mask, map[:,0]*bi_mask, map[:,1]*bi_mask],bi_mask,sm_mask,lmax=3*nside-1,flag='clean', cal_cl=False, n_iter=3, is_Dell=True, method='cutqufitqu',)
