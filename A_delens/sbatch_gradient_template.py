...
##########################################################################
class delensing_Btemp:
    """
    CMB B-mode delensing with template method.
    ...
    * FOR REAL DATA, please modify the code accordingly, where the input (true) maps are not available, and noise (residual) cannot be precisely obtained.
    ...
    """
    def __init__(self,nside,lmax,elmin,elmax,dlmin,dlmax,fil_phi=True,bin_size=40):
        self.nside = nside
        self.lmax = lmax
        self.elmin = elmin
        self.elmax = elmax
    ...
    def prep_map2alm(self):
        ...
        self.elm_noisy,self.blm_noisy = qu2eb_alm(self.nside,self.lmax,Qlm_noisy,Ulm_noisy)     
        self.elm_noise,self.blm_noise = qu2eb_alm(self.nside,self.lmax,Qlm_noise,Ulm_noise)
        self.elm_pure,self.blm_pure = qu2eb_alm(self.nside,self.lmax,Qlm_pure,Ulm_pure)

        self.Plm_noisy = curvedsky.utils.lm_healpy2healpix(Plm_noisy, self.lmax, lmpy=0)
        self.Plm_noise = curvedsky.utils.lm_healpy2healpix(Plm_noise, self.lmax, lmpy=0)
        self.Plm_pure = curvedsky.utils.lm_healpy2healpix(Plm_pure, self.lmax, lmpy=0)
    ...
    def construct_Btemp(self):
        ...
        # Noisy B-mode template
        self.Bmap_noisy,_ = lensingb_mine(self.lmax, self.elmin, self.elmax, self.dlmin, self.dlmax, self.elm_noisy, self.Plm_noisy, nside_t=self.nside, gtype='p',sm_mask=mask,bi_mask=mask)

        # Pure signal B-mode template
        self.Bmap_pure,_ = lensingb_mine(self.lmax, self.elmin, self.elmax, self.dlmin, self.dlmax, self.elm_pure, self.Plm_pure, nside_t=self.nside, gtype='p',sm_mask=mask,bi_mask=mask)

        # Noise B-mode template 
        #self.Bmap_noise1,_ = lensingb_mine(self.lmax, self.elmin, self.elmax, self.dlmin, self.dlmax, self.elm_noise_only, self.Plm_pure, nside_t=self.nside, gtype='p',sm_mask=mask,bi_mask=mask)
        #self.Bmap_noise2,_ = lensingb_mine(self.lmax, self.elmin, self.elmax, self.dlmin, self.dlmax, self.elm_noisy_no_fg, self.Plm_noise, nside_t=self.nside, gtype='p',sm_mask=mask,bi_mask=mask)
        ...
        self.Bmap_noise =  self.Bmap_noisy - self.Bmap_pure
        self.Bmap_noise_differ = (self.Bmap_noise_origin - self.Bmap_noise)
        self.Bmap_noisy_de = (self.Bmap_noisy_origin - self.Bmap_noise)

        _,self.Blen = qu2eb_map(self.nside,self.lmax,self.TQU_map_UNFIL[1],self.TQU_map_UNFIL[2])
        self.Bmap_pure_de = (self.Blen - self.Bmap_pure)
    ...
    def compute_cl(self,):
        ...
        l_bin,dlbb_temp_noisy = get_pseudo_cl(self.nside,mask_cut_sm,'00',self.bin_size,lmax=2000 ,apo=True,f0=[self.Bmap_noisy*mask_cut_sm,self.Bmap_noisy*mask_cut_sm],f2=None,is_Dell=True)      # C^temp
        l_bin,dlbb_temp_noise = get_pseudo_cl(self.nside,mask_cut_sm,'00',self.bin_size,lmax=2000 ,apo=True,f0=[self.Bmap_noise*mask_cut_sm,self.Bmap_noise*mask_cut_sm],f2=None,is_Dell=True)      # ⟨B^tempN B^tempN ⟩
        l_bin,dlbb_pure = get_pseudo_cl(self.nside,mask_cut_sm,'00',self.bin_size,lmax=2000 ,apo=True,f0=[self.Bmap_pure*mask_cut_sm,self.Bmap_pure*mask_cut_sm],f2=None,is_Dell=True)              # ⟨B^tempS B^tempS ⟩
        l_bin,dlbb_temp_cross_bias = get_pseudo_cl(self.nside,mask_cut_sm,'00',self.bin_size,lmax=2000 ,apo=True,f0=[self.Bmap_noise*mask_cut_sm,self.Bmap_pure*mask_cut_sm],f2=None,is_Dell=True)  # ⟨B^tempS B^tempN ⟩


        ######################## For template cross-power spectrum ###############      (Eq.A5)
        l_bin,dlbb_cross_pure = get_pseudo_cl(self.nside,mask_cut_sm,'00',self.bin_size,lmax=2000 ,apo=True,f0=[self.Bmap_pure*mask_cut_sm, self.Bmap_noisy_origin*mask_cut_sm],f2=None,is_Dell=True)       # pure temp * obs map
        l_bin,dlbb_cross = get_pseudo_cl(self.nside,mask_cut_sm,'00',self.bin_size,lmax=2000 ,apo=True,f0=[self.Bmap_noisy*mask_cut_sm, self.Bmap_noisy_origin*mask_cut_sm],f2=None,is_Dell=True)           # C^cross
        l_bin,dlbb_cross_noise2 = get_pseudo_cl(self.nside,mask_cut_sm,'00',self.bin_size,lmax=2000 ,apo=True,f0=[self.Blen*mask_cut_sm, self.Bmap_noise*mask_cut_sm],f2=None,is_Dell=True)                 # ⟨B^lens B^tempN ⟩
        l_bin,dlbb_cross_noise1 = get_pseudo_cl(self.nside,mask_cut_sm,'00',self.bin_size,lmax=2000 ,apo=True,f0=[self.Bmap_noise_origin*mask_cut_sm, self.Bmap_noisy*mask_cut_sm],f2=None,is_Dell=True)    # ⟨Bres,NILC Btemp⟩
        l_bin,dlbb_cross_pure_new = get_pseudo_cl(self.nside,mask_cut_sm,'00',self.bin_size,lmax=2000 ,apo=True,f0=[self.Bmap_pure*mask_cut_sm, self.Blen*mask_cut_sm],f2=None,is_Dell=True)                # ⟨B^lens B^tempS ⟩
        ...
        Bmap_origin = E2Bcorrection_new(self.nside,50,[self.TQU_map_UNFIL[1]*mask_cut_sm, self.TQU_map_UNFIL[1]*mask_cut_sm, self.TQU_map_UNFIL[2]*mask_cut_sm], mask*mask_cut_sm, mask*mask_cut_sm, lmax=3000, flag='clean', cal_cl=False, n_iter=4,is_Dell=True)
        l_bin,dlbb_len_bin = get_pseudo_cl(self.nside,mask_cut_sm,'00',self.bin_size,lmax=2000 ,apo=True,f0=[Bmap_origin*mask_cut_sm,Bmap_origin*mask_cut_sm],f2=None,is_Dell=True)
...
if __name__ == "__main__":
    ...
    delens = delensing_Btemp(nside=2048, lmax=6143, elmin=2, elmax=5000, dlmin=10, dlmax=5000, fil_phi=True, bin_size=40)
