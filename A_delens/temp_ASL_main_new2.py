...
#cl_terms_filename = f"/sharefs/alicpt/users/chenwz/Testarea/FORECAST/A_NEW_WORK/delens/temp_method/cl/terms_check/all_in_one_{file_indices}.dat"
...
def delensing_ones(nside,):
    ...
    mask = hp.read_map(f'/sharefs/alicpt/users/chenwz/Testarea/FORECAST/A_NEW_WORK_MAIN/sim/mask/ALI_LAT_add_SO_LAT_cross_PLANCK_{nside}_sm_thin_0.2.fits')
    mask_sm = hp.read_map(f'/sharefs/alicpt/users/chenwz/Testarea/FORECAST/A_NEW_WORK_MAIN/sim/mask/ALI_LAT_add_SO_LAT_cross_PLANCK_{nside}_sm_thin_0.2.fits')   #smoothed mask is necessary

    mask_cut = hp.read_map(f'/sharefs/alicpt/users/chenwz/Testarea/FORECAST/A_NEW_WORK_MAIN/sim/mask/ALI_LAT_add_SO_LAT_cross_PLANCK_{nside}_sm_thin_0.5.fits')
    mask_cut_sm = hp.read_map(f'/sharefs/alicpt/users/chenwz/Testarea/FORECAST/A_NEW_WORK_MAIN/sim/mask/ALI_LAT_add_SO_LAT_cross_PLANCK_{nside}_sm_thin_0.5.fits')
    ...
    Plm_noisy = hp.map2alm(phi_noisy_part)

    Plm_noise = hp.map2alm(phi_noise_part)

    Plm_pure = hp.map2alm(Pmap_part)
    ...
    elmax=5200          ################注意：这里要手动设定。
    dlmax=5000          ################注意：这里要手动设定。其实取lmax也可以，因为wiener filter起到了截断low SNR的multipole的作用。
    elmin=2
    dlmin=10   

    Bmap_noisy,_ = lensingb_mine(lmax, elmin, elmax, dlmin, dlmax, elm_noisy, Plm_noisy, nside_t=nside, gtype='p',sm_mask=mask,bi_mask=mask)
    #Bmap_noise1,_ = lensingb_mine(lmax, elmin, elmax, dlmin, dlmax, elm_noise_only, Plm_pure, nside_t=nside, gtype='p',sm_mask=mask,bi_mask=mask)
    #Bmap_noise2,_ = lensingb_mine(lmax, elmin, elmax, dlmin, dlmax, elm_noisy_no_fg, Plm_noise, nside_t=nside, gtype='p',sm_mask=mask,bi_mask=mask)
    Bmap_pure,map_pure = lensingb_mine(lmax, elmin, elmax, dlmin, dlmax, elm_pure, Plm_pure, nside_t=nside, gtype='p',sm_mask=mask,bi_mask=mask)
    #Bmap_noise_fg,_ = lensingb_mine(lmax, elmin, elmax, dlmin, dlmax, elm_noisy-elm_noisy_no_fg, Plm_noisy, nside_t=nside, gtype='p',sm_mask=mask,bi_mask=mask)


    ###########################################################################
    ##################       noise & signal post process     ##################

    Bmap_noise =  Bmap_noisy - Bmap_pure
   ...
   ###########################################################################
    ...
    l_bin,dlbb_temp_noisy = get_pseudo_cl(nside,mask_cut_sm,'00',40,lmax=2000 ,apo=True,f0=[Bmap_noisy*mask_cut_sm,Bmap_noisy*mask_cut_sm],f2=None,is_Dell=True)
    l_bin,dlbb_temp_noise = get_pseudo_cl(nside,mask_cut_sm,'00',40,lmax=2000 ,apo=True,f0=[Bmap_noise*mask_cut_sm,Bmap_noise*mask_cut_sm],f2=None,is_Dell=True)
    l_bin,dlbb_pure = get_pseudo_cl(nside,mask_cut_sm,'00',40,lmax=2000 ,apo=True,f0=[Bmap_pure*mask_cut_sm,Bmap_pure*mask_cut_sm],f2=None,is_Dell=True)

    l_bin,dlbb_temp_cross_bias = get_pseudo_cl(nside,mask_cut_sm,'00',40,lmax=2000 ,apo=True,f0=[Bmap_noise*mask_cut_sm,Bmap_pure*mask_cut_sm],f2=None,is_Dell=True)   #NEW CROSS BIAS DUE TO PHI NOISE
    #################cross term is negligible, so we do not calculate it.################


    ######################## For template cross-power spectrum ################
    l_bin,dlbb_cross_pure = get_pseudo_cl(nside,mask_cut_sm,'00',40,lmax=2000 ,apo=True,f0=[Bmap_pure*mask_cut_sm, Bmap_noisy_origin*mask_cut_sm],f2=None,is_Dell=True)  # pure temp * obs map

    l_bin,dlbb_cross = get_pseudo_cl(nside,mask_cut_sm,'00',40,lmax=2000 ,apo=True,f0=[Bmap_noisy*mask_cut_sm, Bmap_noisy_origin*mask_cut_sm],f2=None,is_Dell=True)  # noisy temp * obs map

    l_bin,dlbb_cross_noise2 = get_pseudo_cl(nside,mask_cut_sm,'00',40,lmax=2000 ,apo=True,f0=[Blen*mask_cut_sm, Bmap_noise*mask_cut_sm],f2=None,is_Dell=True)   # signal * noise temp

    l_bin,dlbb_cross_noise1 = get_pseudo_cl(nside,mask_cut_sm,'00',40,lmax=2000 ,apo=True,f0=[Bmap_noise_origin*mask_cut_sm, Bmap_noisy*mask_cut_sm],f2=None,is_Dell=True)  # noise * noisy temp

    l_bin,dlbb_cross_pure_new = get_pseudo_cl(nside,mask_cut_sm,'00',40,lmax=2000 ,apo=True,f0=[Bmap_pure*mask_cut_sm, Blen*mask_cut_sm],f2=None,is_Dell=True)  # pure temp * signal
    ...
    Bmap_origin = E2Bcorrection_new(nside,50,[TQU_map_UNFIL[1]*mask_cut_sm, TQU_map_UNFIL[1]*mask_cut_sm, TQU_map_UNFIL[2]*mask_cut_sm], mask*mask_cut_sm, mask*mask_cut_sm, lmax=3000, flag='clean', cal_cl=False, n_iter=4,is_Dell=True)
    l_bin,dlbb_len_bin = get_pseudo_cl(nside,mask_cut_sm,'00',40,lmax=2000 ,apo=True,f0=[Bmap_origin*mask_cut_sm,Bmap_origin*mask_cut_sm],f2=None,is_Dell=True)
